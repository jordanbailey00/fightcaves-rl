...
static const char* fc_asset_logical_path(const char* path) {
    ...
    marker = strstr(path, "/demo-env/assets/");
    if (marker) return marker + strlen("/demo-env/assets/");
    if (fc_has_prefix(path, "demo-env/assets/"))
        return path + strlen("demo-env/assets/");
...
static int fc_derive_asset_root_from_header(char* out, size_t cap) {
    const char* marker = strstr(__FILE__, "demo-env/src/");
    size_t prefix_len;
    int n;
    if (!marker) return 0;
    prefix_len = (size_t)(marker - __FILE__);
    n = snprintf(out, cap, "%.*sdemo-env/assets", (int)prefix_len, __FILE__);
    return n > 0 && (size_t)n < cap;
}
...
static int fc_derive_repo_root_from_asset_root(char* out, size_t cap,
    ...
    marker = strstr(asset_root, "/demo-env/assets");
    if (!marker && fc_has_prefix(asset_root, "demo-env/assets")) {
...
static const char* fc_asset_root(void) {
    ...
    fc_copy_path(default_root, sizeof(default_root), "demo-env/assets");
    return default_root;
}
